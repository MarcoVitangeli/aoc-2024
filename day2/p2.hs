...
checkIsSafeAny prev (x:xs) | checkIsSafe (prev ++ xs) Nothing Nothing = True
                           | otherwise = checkIsSafeAny (prev ++ [x]) xs 

checkIsSafeAny _ [] = False

main :: IO ()
main = do
    lines <- DC.parseInputFile "input.txt"
    print $ length $ filter (\l -> checkIsSafe l Nothing Nothing || checkIsSafeAny [] l) lines